...
class ProviderContractTest {
    ...
    private val versionName = "0.1.0-preview3"
    ...
    fun negotiationBuilderProducesExactContractFields() {
        ...
        assertEquals("translated-voice-overlay", info.providerId)
        assertEquals(packageName, info.packageName)
        assertEquals(versionName, info.versionName)
        assertEquals(listOf("DUB_ARTIFACT_V1@1"), info.capabilities)
    ...
    fun fallbackVersionMatchesPreviewVersion() {
        assertEquals("0.1.0-preview3", ProviderContract.VERSION_NAME)
    }
}
