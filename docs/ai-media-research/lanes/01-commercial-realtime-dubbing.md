...
The best direct player-side pilot candidate is **Qwen3.5-LiveTranslate-Flash-Realtime**, because it is the only direct S2ST API in this repository with a successful transport/timing smoke test and it has explicit token pricing, tentative/confirmed text, hotwords, spoken output in 29 languages, and clone modes. **OpenAI GPT-Realtime-Translate** is the cleanest integration fallback, and **Gemini 3.5 Live Translate** is the broad-language fallback. For managed broadcast ingest and packaged outputs, **CAMB.AI** leads the documented feature set. **Palabra** remains the strongest explicit voice-preserving specialist with broadcast packaging. These are pilot orders, not production winners. No candidate has a representative common-corpus or long-form result.
...
**So what.** For direct sidecar translation, prioritize Qwen, OpenAI and Gemini. For managed broadcast packaging, prioritize CAMB, then Deepdub and Palabra. Do not let fresh STT/TTS announcements (Gemini Transcribe, Deepgram Flux, Sarvam Realtime) masquerade as one-call live speech translation.
...
|---|---|---|---|---|---|
| **Qwen3.5 LiveTranslate Flash Realtime** | Model Studio WebSocket/AOQ/WebRTC; audio required, optional video; translated text and audio arrive incrementally with confirmed text plus tentative `stash`. A separate host packager is required for HLS/DASH/broadcast. | 60 speech-input/text-output languages and 29 spoken outputs; fixed voices, pre-cloned voices, clone-once and clone-every-response modes. | International Model Studio key stays in the host credential broker. No client-safe ephemeral token was verified. | $7.50/M input-audio tokens and $30/M output-audio tokens; 7 input and 12.5 output tokens/second. Equal-duration estimate is $0.02565/source min. The observed expanded-output probes cost about $0.0374 to $0.0384/source min before optional output text. | Official claim as low as 2.8 s. Local clean 9.814 s probe: first translated audio 2.557 s from probe start with fixed voice and 4.578 s in a confounded clone/ASR configuration. Not a common-corpus or playback-synchronization result. [model](https://www.alibabacloud.com/help/en/model-studio/qwen3-5-livetranslate-flash-realtime) · [pricing](https://www.alibabacloud.com/help/en/model-studio/model-pricing) · [receipt](../benchmarks/2026-08-29-qwen-live-translate.json) |
...
**So what.** Qwen is the first direct player-side pilot because it has the only successful local direct-S2ST smoke test and calculable pricing. OpenAI and Gemini are the clean direct alternatives. CAMB is the managed broadcast leader, not the default player-side winner. Palabra has stronger explicit voice/broadcast controls but a pricing inconsistency to resolve. DeepL has excellent key isolation and reconnect semantics but does not verify source-voice preservation.
...
These are pilot orders, not production selections. No candidate has a common representative corpus, invoice reconciliation and long-form sync result.

### Direct player or media-worker translated audio

1. **Qwen3.5 LiveTranslate, trial leader.** Only locally smoke-tested direct S2ST candidate; explicit tentative/confirmed text, 29 spoken outputs, clone modes and calculable output-duration-sensitive price. Risks: one synthetic test, output expanded 1.52 to 1.57 times, no long-form drift or voice-similarity score, and no verified client-safe ephemeral credential.
2. **OpenAI GPT-Realtime-Translate, integration fallback.** Dedicated translation endpoint, WebRTC for Android/browser clients, WebSocket for media workers, progressive audio/transcript deltas, short-lived client secrets, and $0.034/min. Official cookbook says over 70 input and 13 output languages; exact pair acceptance still needs runtime probing. Risks: no local latency/quality test and no verified identity preservation.
3. **Gemini 3.5 Live Translate Preview, broad-language fallback.** 70+ languages, explicit PCM/chunk mechanics, progressive audio and constrained ephemeral tokens at about $0.0368/min. Risks: preview status and documented voice replication, gender and multi-speaker inconsistencies.
4. **CAMB.AI realtime S2S.** Valid direct socket with selectable cloned voices. Risks: beta, 14 listed realtime languages, no comparable test, no public unit economics, and no verified client-safe ephemeral credential.
5. **Palabra or DeepL Voice.** Specialist controls for voice preservation or secure session/reconnect behavior; both need direct cost and quality validation.

### Managed live broadcast packaging

1. **CAMB Streaming.** Native SRT ingest and SRT/RTMP/HLS outputs, with automatic source-speaker cloning when voice IDs are omitted. Price and long-run latency remain gates.
2. **Deepdub Live.** Native broadcast challenger, but early access, quote-only and unmeasured.
3. **Palabra Broadcast.** Plausible voice and protocol specialist; price, wire behavior and SLA need direct validation.

### Asynchronous whole-title dubbing

1. **CAMB End-to-End Dubbing** for direct self-serve multi-target job mechanics.
2. **Dubformer** for explicit project/output controls and soundalike options.
3. **Rask, Sarvam, Deepdub Managed and RWS/Papercup** as plan-, region- or service-constrained controls.
...
|---|---|---|---|---|---|---|
| Qwen LiveTranslate | L/P | WS/AOQ/WebRTC PCM → audio plus confirmed/tentative text | 60 text / 29 spoken | Fixed or clone modes | $0.02565/min at equal duration; observed smoke estimates $0.0374 to $0.0384/source min | 2.8 s vendor claim; one limited local smoke test |
...
1. **Run a four-vendor direct live bake-off:** Qwen, CAMB, OpenAI and Gemini. Add Palabra if preserved speaker identity is a hard launch requirement. Test identical clean/noisy, single/multi-speaker and code-switch clips for EN↔ES, EN↔FR plus NuvioTV priority pairs.
...
- Vendor account/API calls in this documentation-only lane were not executed. The repository's separate Qwen benchmark lane did execute authenticated probes and retains a sanitized receipt. Every other vendor still requires an authenticated pilot to verify catalogs, quotas, pricing ledgers and actual responses.
